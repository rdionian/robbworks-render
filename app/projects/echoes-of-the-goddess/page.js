"use client";
import TechnicalLog from "@/components/TechnicalLog/technical-log";
import ImageGrid from "@/components/Images/image-grid";
import Logbook from "@/components/Logbook/logbook";

export default function EchoesOfTheGoddess() {
  return (
    <main className="relative min-h-screen overflow-x-hidden">
      {/* Green/sky gradient background */}
      <div
        className="absolute inset-0 min-h-full -z-10 pointer-events-none"
        style={{
          background: "linear-gradient(to top, #000000 0%, #0b3d26 10%, #1c7d3f 35%, #5bd66b 55%, #76d9f0 75%, #9be4ff 100%)",
      ...
      <div className="relative z-10">
        ...
        <section className="relative w-full block overflow-hidden m-0 p-0">
          <a
            href="/"
            className="absolute top-3 left-3 z-10 bg-black/50 border border-white/[0.12] px-3 py-1.5 text-[#76d9f0] rounded-md text-[1.1rem] no-underline hover:scale-105 transition-transform"
          >
            ← Home
          </a>
          ...
          <img
            src="/DevHero.png"
            alt="Echoes of the Goddess hero"
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center text-white z-[2] m-0 p-0 max-[900px]:px-4 max-[600px]:px-3" style={{ textShadow: "2px 2px 8px rgba(0,0,0,0.8)" }}>
            <h1 className="text-[4rem] m-0 max-[900px]:text-[2rem] max-[600px]:text-[2.5rem]">Echoes of the Goddess</h1>
            <p className="text-[1.5rem] m-0 max-[900px]:text-[1.05rem] max-[600px]:text-[1rem]">
              An immersive narrative experience of memory, loss, and divinity.
            </p>
            <button className="hidden bg-transparent border-[3px] border-[#00bfff] rounded-lg text-white font-bold px-8 py-3 cursor-pointer opacity-60 hover:scale-105 hover:shadow-[0_0_20px_#00bfff] transition-all mt-4" disabled>
              ▶ Play Demo
            </button>
          </div>
        </section>
        ...
        {/* Promo Images */}
        <ImageGrid images={["/echoes1.png", "/echoes2.png", "/echoes3.png", "/echoes4.png"]} />

        {/* Logbook */}
        <section className="flex justify-center items-center p-0 mx-auto text-center max-w-[1200px] w-full bg-transparent">
          <Logbook project="echoes-of-the-goddess" />
        </section>

        {/* Technical Log */}
        <section className="p-0 m-0 bg-transparent text-center">
          <TechnicalLog />
        </section>
        ...
        <footer className="text-[#666] text-center py-6 px-4 mt-20 relative" style={{ backgroundColor: "transparent" }}>
          ...
          <div className="flex justify-center items-center gap-2 mt-3">
            <a href="https://www.linkedin.com/in/robert-dionian-a08739235" target="_blank" rel="noopener noreferrer" title="LinkedIn" className="inline-flex items-center justify-center hover:scale-110 transition-transform">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#76d9f0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
